...
if ($searchQuery !== '') {
    ...
    // Check if student already has active document access
    if ($searchResult) {
        $now = date('Y-m-d H:i:s');
        $ea = $db->prepare("SELECT id, expires_at, note, granted_at FROM user_edit_access WHERE user_id = ? AND scope = 'documents' AND is_active = 1 AND expires_at > ? LIMIT 1");
...
$activeGrants = $db->prepare("
    SELECT ea.id, ea.user_id, ea.granted_at, ea.expires_at, ea.note, u.email, u.username
